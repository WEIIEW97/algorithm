...
takes to complete the (i+1)th course. You must find the minimum number of months
needed to complete all the courses following these rules:
...
int minimumTimeV1(int n, vector<vector<int>>& relations, vector<int>& time) {
  vector<vector<int>> G(n);
  vector<int> indegree(n), dist(n);
  for (auto& e : relations) { // build graph and count indegrees
  ...
  for (int i = 0; i < n; i++) {
    if (indegree[i] == 0) { // enqueue nodes with 0 indegree.
      q.push(i);
      dist[i] = time[i]; // source nodes' distance is their corresponding time
    }
  }
  while (q.size()) {
    int u = q.front();
    q.pop();
    for (auto& v : G[u]) {
      dist[v] = max(dist[u] + time[v],
                    dist[v]); // update the distance of node `v` using the
                              // maximum distance of predecessor nodes.
      if (--indegree[v] == 0)
        q.push(v); // enqueue node `v` when its indegree drops to 0
    }
  }
  return *max_element(begin(dist), end(dist));
}

// DFS+memorization(Post-order traversal)
int minimumTimeV2(int n, vector<vector<int>>& relations, vector<int>& time) {
  vector<vector<int>> G(n);
  vector<int> dist(n);
...
int main() {
  ...
  auto ans = minimumTimeV1(n, relations, time);
  cout << ans << endl;
  return 0;
}